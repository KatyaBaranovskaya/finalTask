...
CREATE TABLE IF NOT EXISTS `hotel`.`room_types` (
  ...
  `type_name` VARCHAR(45) NOT NULL COMMENT 'Поле содержит тип номера (люкс, полулюкс и тд).',
  ...
  PRIMARY KEY (`id_type`),
  INDEX `type_idx` (`type_name` ASC)  COMMENT 'Индекс, который состоит из типа номера, т.к по данному полю наиболее часто выполняется запрос на чтение.',
...
COMMENT = 'Данная таблица предназначена для хранения данных о видах номеров отеля. Таблица содержит первичный ключ, который хранит в себе \"ID типа  номера\".';
...
CREATE TABLE IF NOT EXISTS `hotel`.`rooms` (
  `room_number` INT(11) UNSIGNED NOT NULL COMMENT 'Поле содержит номер апартамента отеля.',
  ...
  INDEX `status_idx` (`status` ASC)  COMMENT 'Индекс, который состоит из статуса номера, т.к по данному полю наиболее часто выполняется запрос свободен номер или нет.',
  INDEX `id_type_fk_idx` (`id_type` ASC),
  CONSTRAINT `id_type_fk`
    FOREIGN KEY (`id_type`)
    REFERENCES `hotel`.`room_types` (`id_type`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8
COMMENT = 'Данная таблица предназначена для хранения данных о номерах отеля. Таблица содержит первичный ключ, который хранит в себе номер апартамента, а также поля тип номера и его статус. Все поля данной таблицы являются не нулевыми.';
...
CREATE TABLE IF NOT EXISTS `hotel`.`users` (
  ...
  `date_birth` DATE NOT NULL COMMENT 'Поле содержит дату рождения клиента.',
  `telephone` VARCHAR(45) NOT NULL COMMENT 'Поле содержит телефон клиента.',
...
AUTO_INCREMENT = 21
DEFAULT CHARACTER SET = utf8
COMMENT = 'Данная таблица предназначена для хранения персональных данных о всех клиентах. Таблица содержит первичный ключ - \"ID клиента\". ';
...
CREATE TABLE IF NOT EXISTS `hotel`.`orders` (
  ...
  `type_apartment` VARCHAR(45) NOT NULL,
...
AUTO_INCREMENT = 31
DEFAULT CHARACTER SET = utf8
COMMENT = 'Данная таблица предназначена для хранения обработанных заявок клиентов, т.е. каждая заявка уже подтверждена администратором и имеет статус заказа. Таблица содержит первичный ключ -\"ID заказа\". ';
